...
def get_obs_state(model: mujoco.MjModel, data: mujoco.MjData, site_left_id: int, site_right_id: int) -> np.ndarray:
    """
    observation.state 예시:
    [L_pos(3), L_quat_wxyz(4), R_pos(3), R_quat_wxyz(4)] => (14,)
    """
    lpos, lquat = site_pose(model, data, site_left_id)
    rpos, rquat = site_pose(model, data, site_right_id)
...
def main():
    ...
    with mujoco.viewer.launch_passive(model=model, data=data, show_left_ui=False, show_right_ui=False) as viewer:
        ...
        while viewer.is_running() and episode_id < NUM_DEMO:
            ...
            # step env (physics) - teleop/IK loop
            frame = teleop.read()
            ...
            if rec_accum >= REC_DT:
                ...
                # action label: 가장 안전한 기본은 "현재 qpos"를 저장
                # (원하면 last_vel / data.ctrl / target pose 등으로 바꿀 수 있음)
                action_qpos = data.qpos.copy()
                
                # 추가로 목표(mocap)도 같이 저장하면 BC 학습 때 유용함
                    ...
                    dataset.add_frame(
                        {
                            ...
                            # "action.vel": np.array(last_vel, dtype=np.float64) if last_vel is not None else None,
                        },
                        task=TASK_NAME,
                    )
